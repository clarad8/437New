...
import { Button, TextField, Chip, Typography, Box, IconButton, ButtonGroup, Grid } from '@mui/material';
...
const FirestoreDiscussionComponent = () => {
  const [firestoreDiscussions, setFirestoreDiscussions] = useState<FirestoreDiscussion[]>([]);
    ...
    if (selectedPost) {
      return (
        <div>
          ...
          <Box style={{ marginBottom: '20px', border: '2px solid #2196f3', padding: '10px', borderRadius: '5px', cursor: 'pointer' }}>
            <Typography variant="h6" >Title: {selectedPost.title}</Typography>
            <Typography variant="body1">Type: {selectedPost.type}</Typography>
            <Typography variant="body1">Content: {selectedPost.content}</Typography>
          </Box>
  
          {/* Container for Student's Response */}
          <Box style={{ marginBottom: '20px', border: '2px solid #2196f3', padding: '10px', borderRadius: '5px' }}>
            ...
            />
            <Button variant="contained" color="primary" onClick={handleStudentResponseSubmit}>
              Submit Student's Response
            </Button>
          </Box>
  
          {/* Container for Tutor's Response */}
          <Box style={{ border: '2px solid #2196f3', padding: '10px', borderRadius: '5px' }}>
            ...
            <TextField
              ...
              style={{ marginBottom: '10px' }}
            />
            <Button variant="contained" color="primary" onClick={handleTutorResponseSubmit}>
              Submit Tutor's Response
            </Button>
          </Box>
  
          {/* Button to go back to posts */}
          <Button variant="outlined" color="primary" onClick={handleBackToPosts} style={{ marginTop: '30px' }}>
        ...
        <div>
          ...
          <Box>
            ...
            <ButtonGroup color="primary" style={{ marginRight: 10 }}>
              <Button
                ...
                variant={postVisibility === 'everyone' ? 'contained' : 'outlined'}
              >
                Everyone
              </Button>
              <Button
                onClick={() => setPostVisibility('students')}
                variant={postVisibility === 'students' ? 'contained' : 'outlined'}
              >
                Students
              </Button>
              <Button
          ...
          <Box>
            {firestoreDiscussions
              .filter((discussion) => !selectedClass || discussion.class === selectedClass)
              .map((discussion) => (
                <Chip key={discussion.id} label={discussion.content} style={{ margin: '5px' }} />
              ))}
          </Box>
          ...
          <Box>
            <Button variant="contained" color="primary" onClick={handleCreateFirestoreDiscussion} style={{ marginTop: 10 }}>
              Post New Discussion
            </Button>
          </Box>
        </div>
  ...
  const handleCreateFirestoreDiscussion = async () => {
    if (newDiscussion.trim() !== '' && selectedClass && postTitle.trim() !== '') {
  ...
  const handleClassButtonClick = (selectedClass: string) => {
    ...
    const selectedClassData = pastPosts.find(post => post.id === selectedClass);
  ...
  return (
    ...
    <Box style={{ marginLeft: '30px' }}>
      ...
      <Box>
        {['CSE330', 'CSE217', 'CSE231', 'CSE240', 'CSE256', 'CSE311', 'CSE411', 'CSE412', 'CSE417', 'CSE433', 'CSE131', 'CSE247', 'CSE437', 'CSE332', 'CSE132', 'CSE361', 'CSE347', 'CSE204'].map((className) => (
          <Button
            key={className}
            variant="outlined"
            ...
            style={{
              marginRight: 10,
              marginBottom: 10,
              backgroundColor: selectedClass === className ? '#2196f3' : 'white',
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={8} style={{ marginLeft: '30px' }}> {/* Right side: 3/4 of the page */}
